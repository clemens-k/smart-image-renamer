...
def get_exif_data(img_file: str) -> dict:
    ...
    try:
        ...
        try:
            exif_data = {TAGS[k]: v for k, v in img.getexif().items() if k in TAGS}
        except AttributeError:
        ...
        for root, dirs, files in os.walk(input_path):
            ...
            for f in sorted(files):
                ...
                try:
                    ...
                    try:
                        ...
                        if old_file_name.split(".")[-1] in ["mp4", "mov", "3gp"]:
                            print(
                                f"WARNING: {old_file_name} is a video file, but I cannot open it!"
                            )
                            if (
                                process_unreadable_videos != BulkChoice.NEVER
                                and process_unreadable_videos != BulkChoice.ALWAYS
                            ):
                                process_unreadable_videos = get_bulk_choice(
                                    f"Process file time stamp {old_file_name}?"
                                )
                            if process_unreadable_videos == BulkChoice.ABORT:
                                sys.exit(1)
                            elif (
                                process_unreadable_videos == BulkChoice.NEVER
                                or process_unreadable_videos == BulkChoice.NO
                            ):
                                continue
                            elif (
                                process_unreadable_videos == BulkChoice.ALWAYS
                                or process_unreadable_videos == BulkChoice.YES
                            ):
                                tag_data = get_file_data(old_file_name)
                        elif verbose and not quiet:
                            print(
                                f"INFO: {old_file_name} is neither an image nor a video file!"
                            )
                            continue
                except NoExifTimeStamp:
                    if not quiet:
                        print(
                            f"INFO: {old_file_name} does not contain usable timestamp in EXIF!"
                        )

                    # Decide how to proceed with image file, that has no usable EXIF timestamp
                    if (
                        ignore_exifless != BulkChoice.NEVER
                        and ignore_exifless != BulkChoice.ALWAYS
                    ):
                        ignore_exifless = get_bulk_choice(
                            f"Ignore file {old_file_name} without EXIF timestamp?"
                        )

                    if ignore_exifless == BulkChoice.ABORT:
                        sys.exit(1)
                    elif (
                        ignore_exifless == BulkChoice.ALWAYS
                        or ignore_exifless == BulkChoice.YES
                    ):
                        continue
                    elif (
                        ignore_exifless == BulkChoice.NEVER
                        or ignore_exifless == BulkChoice.NO
                    ):
                        try:
                ...
                if old_file_name == new_file_name_complete:
                    if verbose and not quiet:
                        print(
                            f"INFO: Skipping {old_file_name}, because it does already have correct file name!"
                        )
                    continue
